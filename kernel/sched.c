...
struct runqueue {
	 ...
	 * 	包括两个prio_array_t结构的数组,每个数据结构都表示一个可运行进程的集合.
...
static void recalc_task_prio(task_t *p, unsigned long long now)
{
	...
	if (likely(sleep_time > 0)) {
		 ...
		 * 下面三个判断的意义是:
		 * 	不是内核线程,
		 * 	从TASK_UNINTERRUPTIBLE状态被唤醒,
		 * 	进程连续睡眠时间是否超过给定的睡眠时间极限
		 * */
...
out_activate:
	...
	/*
	 * 如果进程处于TASK_UNINTERRUPTIBLE状态,函数递减目标运行队列的nr_uninterruptible,并把p->activated置为1
	 * ???????????????????????????为什么
	 * */
...
void fastcall sched_fork(task_t *p)
{
	...
	/*父进程将自己时间片中剩余的节拍数的一半分给自己,避免用户获得无限的CPU时间*/
...
task_t * context_switch(runqueue_t *rq, task_t *prev, task_t *next)
{
	...
	/*next是普通进程,用next地址空间替换prev的地址空间*/
	} else
...
/*
 * 检查是否调度域处于严重的不平衡状态,更确切的说,检查是否可以通过把最繁忙的组中的一些进程迁移到本地CPU的运行队列来减轻不平衡的状态
...
/*
 * EXPIRED_STARVING检查运行队列中的第一个过期进程的等待时间是否已经超过1000个时钟节拍乘以运行运行队列中的可运行进程数加1,若是,宏产生1
 * 如果当前进程的静态优先级大于一个过期进程的静态优先级,EXPIRED_STARVING也产生值1.
 * */
			 ...
			 cputime_t cputime)
{
	...
	/*更新当前进程描述符的stime,只在父进程询问它的其中一个子进程的状态时才对cstime和cutime更新*/
...
void scheduler_tick(void)
{
	...
	/*把转换为纳秒的TSC的当前值存入本地运行队列的timestamp_last_tick.这个时间戳是从函数sched_clock()获得*/
	rq->timestamp_last_tick = sched_clock();

	/*检查当前进程是否是本地CPU的swapper进程*/
	if (p == rq->idle) {
		/*本地除了swapper外,还有另外一个可运行的进程,就设置当前进程的TIF_NEED_RESCHED,以强迫进行重新调度*/
	 ...
	 * 若不是,说明进程已经过期,但是还没有被替换,设置TIF_NEED_RESCHED标志,以强制进行重新调度
	 * */
	...
	if (rt_task(p)) {
		 ...
		 */
		  ...
		  * 如果确定时间片确实用完了,执行一系列操作达到抢占当前进程的目的,有必要的话，就尽快的抢占
		  * */
		if ((p->policy == SCHED_RR) && !--p->time_slice) {
			/*重填进程的时间片计数器.该函数检查进程的静态优先级,并根据公式返回相应的基本时间片*/
			p->time_slice = task_timeslice(p);
			/*该标志被fork()系统调用服务例程中的copy_process()设置,并在进程的第一个时间片刚一用完时立刻清0*/
			p->first_time_slice = 0;
			/*设置进程的TIF_NEED_RESCHED标志,该标志强制调用schedule()函数,以便current指向的进程能被另外一个有相同优先级(更高)的实时进程所取代*/
			set_tsk_need_resched(p);
			 ...
			 * 把current指向的进程放到链表的尾部,可以保证在每个优先级与它相同的可运行实时进程获得CPU时间片以前,它不会再次被选择来执行.
			 * */
	...
	if (!--p->time_slice) {
		...
		/*若本地运行队列的expired_timestamp==0(过期进程集合为空),把当前时钟节拍值赋给expired_timestamp*/
		...
		/*时间片没有用完,检查当前进程的剩余时间片是否太长,具有高静态优先级的交互式进程,其时间片被分成大小为TIMESLICE_GRANULARITY的几个片段*/
...
/*
 * 内核如果支持超线程技术,检查要被选中执行的进程,其优先级是否比已经在相同物理CPU的某个逻辑CPU上运行的兄弟进程低
 * */
...
/*
 * 实现调度程序
 * 从运行队列的链表中找到一个进程,并随后将CPU分配给这个进程,schedule()可以由几个内核控制路径调用,可以采用直接调用或延迟调用的方式
 * */
...
need_resched_nonpreemptible:
	...
	/*
	 * 计算prev所用放入CPU时间片长度
	 * 通常要限制在1s的时间,runtime的值用来限制进程对CPU的使用
	 * */
	...
	/*鼓励进程具有较长的平均睡眠时间,CURRENT_BONUS返回0-10之间的值,它与进程的平均睡眠时间是成正比的*/
...
go_idle:
		/*没有可运行进程时,从另外一个运行队列迁移一些可运行进程,与load_balance()类似*/
		idle_balance(cpu, rq);
		if (!rq->nr_running) {
			next = rq->idle;
	...
	if (unlikely(!array->nr_active)) {
		...
		/*交换运行队列数据结构的active和expired字段,所有的过期进程变为活动进程，空集合接纳将要过期的进程*/
	...
	if (!rt_task(next) && next->activated > 0) {
		...
		/*
		 * 把进程插入运行队列开始所经过的纳秒数加到进程的平均睡眠时间中,换言之,进程的睡眠时间被增加了,以包括进程在运行队列中等待CPU所消耗的时间
		 * 注意:
		 * 	1.只增加等待时间的部分
		 * 	2.增加全部运行队列的等待时间,因为交互式进程更可能被异步事件而不是同步事件唤醒
		 * */
		if (next->activated == 1)
...
switch_tasks:
	...
	if (likely(prev != next)) {
		...
		finish_task_switch(prev);
	/*prev和next是同一个进程,当前运行队列中没有优先级较高或相等的其他活动进程*/
	} else
...
void fastcall __sched wait_for_completion(struct completion *x)
{
	...
	if (!x->done) {
		...
		/*done >0 终止,说明complete()已经在另外一个CPU运行,否则将current作为互斥队列加入到等待队列末尾*/
		do {
...
/*
 * 只影响调用它的进程
 * 参数:int increment---用来修改进程描述符的nice字段,负值相当于请求优先级增加,并请求超级用户特权,其正值相当于请求优先级减少
 * */
 ...
 * 但调度程序把它放在运行队列的过期进程集合中(普通进程),或放在运行队列链表末尾(实时进程),具有相同动态优先级的其他进程将有机会运行,主要由SCHED_FIFO使用
 * */
...
/*
 * 参数:pid_t pid---实时进程的轮转时间片写入用户态地址空间的一个结构中.pid==0,写当前进程时间片
 * */
asmlinkage
