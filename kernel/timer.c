...
struct tvec_t_base_s {
	...
	/*
	 * 需要检查的动态定时器的最早到期时间,
	 * 	timer_jiffies == jiffies--->可延迟函数没有积压
	 * 	timer_jiffies < jiffies--->前几个节拍相关的可延迟函数必须处理
	 * 系统启动时,timer_jiffies == jiffies,且只能由run_timer_softirq()增加
	 * */
	unsigned long timer_jiffies;
	/*SMP中,指向由本地CPU当前正处理动态定时器的timer_list数据结构*/
	struct timer_list *running_timer;
	/*包含一个vec数组,由256个list_head元素组成(256个动态定时器链表组成),这个结构包含了在紧接着到来的255个节拍内将要到期的所有动态定时器*/
	tvec_root_t tv1;
	/*tv2,tv3,tv4包含在紧接着到来的2^14-1,2^20-1,2^26-1个节拍内将要到期的所有动态定时器*/
	tvec_t tv2;
	tvec_t tv3;
	tvec_t tv4;
	/*与前面的区别,vec数组最后一项是一个大的expires字段值的动态定时器链表.不需要从其他数组补充*/
	tvec_t tv5;
...
/*
 * 从链表中删除定时器,然后检查定时器函数是否还在其他CPU上运行,如果是,该函数等待,直到定时器函数结束
 * 要考虑定时器函数重新激活自己
 * */
...
/*
 * 如果定时器函数不重新激活定时器,使用该函数使定时器无效,并等待直到定时器函数结束
 * */
...
static inline void __run_timers(tvec_base_t *base)
{
	...
	/*base->timer_jiffies > jiffies时停止.一般情况下,该循环会连续执行jiffies-base->timer_jiffies+1次*/
	while (time_after_eq(jiffies, base->timer_jiffies)) {
		struct list_head work_list = LIST_HEAD_INIT(work_list);
		struct list_head *head = &work_list;
		/*计算base->tv1中链表的索引,该索引保留着下一次要处理的定时器,TVR_MASK == 1111 1111*/
...
repeat:
	...
	/*所有到期的定时器已经被处理了.SMP中,base->running_timer == NULL*/
...
/*
 * 更新一些内核统计数,单处理器全局中断处理程序或多处理器系统上的本地时钟中断处理程序调用
 * */
...
/*
 * 与TIMER_SOFTIRQ相关的可延迟函数,软定时器的处理
 * */
...
/*
 * 更新xtime变量的值
 * 用户程序从xtime中获得当前时间和日期,内核周期性的更新该变量,才能使它的值保持相当的精确,此时已经获得写操作的xtime_lock顺序锁
 * */
static inline void update_times(void)
{
	...
	/*
	 * wall_jiffies---xtime最后更新的时间
	 * wall_jiffies可以小于jiffies-1,因为一些定时器中断会丢失,也就是,内核不必每个时钟节拍更新xtime,但是最后不会有时钟节拍丢失
	 * xtime---最终存放正确的系统时间,对丢失的定时器中断的检查在cur_timer的mark_offset中完成
	 * */
	...
	/*计算处于TASK_RUNNING或TASK_UNINTERRUPTBLE状态的进程数,并用这个数据更新平均系统负载*/
	calc_load(ticks);
}
...
asmlinkage long sys_nanosleep(struct timespec __user *rqtp, struct timespec __user *rmtp)
{
	...
	/*
	 * 进程延迟
	 * expire == 0--->进程延时到期,系统调用结束,否则,系统调用自动重新启动
	 * */
