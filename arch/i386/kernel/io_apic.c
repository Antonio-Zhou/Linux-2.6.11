...
/*
 * 实现CPU的IRQ亲和力:修改I/O APIC 的中断重定向表表项.可以把中断信号发送到某个特定的CPU上
 ...
 * 如果该函数发现负荷最重的CPU和负荷最轻的CPU之间IRQ负载不平衡的问题太严重.它要么把IRQ从一个CPU转移到另外一个CPU,要么让所有的IRQ在所有CPU之间"轮转"
 * */
...
void __init setup_IO_APIC(void)
{
	...
	sync_Arb_IDs();
	/*在系统启动过程中,初始化I/O APIC芯片,芯片的中断重定向表的24项被填充,以便根据"最低优先级" 模式把来自I/O 硬件设备的所有信号都传递给系统中的每个CPU*/
	setup_IO_APIC_irqs();
	init_IO_APIC_traps();
	check_timer();
	if (!acpi_ioapic)
