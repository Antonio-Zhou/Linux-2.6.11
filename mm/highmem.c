 ...
 * == 1---对应的页表项没有映射任何高端内存页框,但是它不能使用,因为自从它最后一次使用以来,其相应的TLB表现还未被刷新
...
/*
 * 把页框的物理地址插入到pkmap_page_table的一个项中,并在page_address_htable散列表中加入元素
 * */
...
start:
	...
	for (;;) {
		...
		/*如果在pkmap_count没有找到空的计数器,阻塞当前进程,直到某个进程释放pkmap_page_table页表中的一个表项*/
		{
			...
			/*进程被唤醒,检查是否存在另外一个进程已经映射该页,如果还没有其他进程映射该页,则内循环重新开始*/
...
void fastcall *kmap_high(struct page *page)
{
	...
	/*保护页表免受多处理器系统上的并发访问,没有必要禁止中断,因为中断处理程序和可延迟函数不能调用kmap()*/
	spin_lock(&kmap_lock);
	/*是否已经被映射*/
...
void fastcall kunmap_high(struct page *page)
{
	...
	case 0:
		BUG();
	/*没有进程在使用页,唤醒由map_new_virtual()添加在等待队列中的进程*/
	case 1:
		/*
...
/*
 * 返回页框对应的线性地址,如果页框在高端内存中,并没有被映射,返回NULL
 * */
