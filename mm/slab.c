...
/*
 * 分别指向26个高速缓存描述符,与其相关的内存区大小为2^5--2^17字节
 * 每种大小都有两个高速缓存:一个适用于ISA DMA分配,另一个用于常规分配
 * */
...
/*cache_reap()的地址存放在reap_work的func字段*/
 ...
 */
  ...
  * 参数:kmem_cache_t *cachep---指向需要额外页框的高速缓存的高速缓存描述符，请求页框的个数由存放在cachep->gfporder字段中的order决定
  * 	 int flags---说明如何请求页框,这组标志与存放在高速缓存描述符的gfpflags字段中的专用高速缓存分配标志相结合
  * 	 int nodeid
  * */
...
/*
 * 撤销slab,并释放相应的页框到分区页框分配器
 * 条件:1.slab高速缓存中有太多的空闲对象
...
static int cache_grow (kmem_cache_t * cachep, int flags, int nodeid)
{
	...
	/* Make slab active. */
	/*
	 * 将新得到的slab描述符添加到高速缓存描述符*cachep的全空slab链表的末端
	 * 并更新高速缓存中的空闲对象计数器
	 * */
...
retry:
	...
	/*试图填充本地高速缓存,填充值为高速缓存的slab中包含的多达ac->batchount个空闲对象的指针*/
	while (batchcount > 0) {
		struct list_head *entry;
		struct slab *slabp;
		/* Get slab alloc is to come from. */
		/*查看高速缓存描述符的slabs->partial和slabs_free链表*/
		...
		while (slabp->inuse < cachep->num && batchcount--) {
			...
			/* get obj pointer */
			/*
			 * 对于slab中的每个空闲对象,函数增加slab描述符的inuse.
			 * 将对象的地址插入本地高速缓存,并更新free字段使得它存放了slab中下一个空闲对象的下标
			 * */
...
must_grow:
	/*
	 * 被加到本地高速缓存上的指针个数被存放在ac->avail:
	 * 函数递减同样数量的kmem_list3结构的free_objects字段来说明这些对象不再空闲
	 * */
	l3->free_objects -= ac->avail;
alloc_done:
	spin_unlock(&cachep->spinlock);

	/*ac->avail >0 说明一些高速缓存再填充的情况发生了*/
	if (unlikely(!ac->avail)) {
		int x;
		/*获得一个新slab,从而获得新的空闲对象*/
...
static inline void * __cache_alloc (kmem_cache_t *cachep, int flags)
{
	...
	/*
	 * 试图从本地高速缓存获得一个空闲对象
	 * avail包含指向最后被释放的对象的项在本地高速缓存中的下标
	 * */
	if (likely(ac->avail)) {
		STATS_INC_ALLOCHIT(cachep);
		ac->touched = 1;
		/*获得那个空闲对象的地址,并递减ac->avail*/
		objp = ac_entry(ac)[--ac->avail];
	} else {
		STATS_INC_ALLOCMISS(cachep);
		/*没有空闲对象时,重新填充本地高速缓存并获得一个空闲对象*/
...
static void free_block(kmem_cache_t *cachep, void **objpp, int nr_objects)
{
	...
	for (i = 0; i < nr_objects; i++) {
		...
		/*
		 * 确定包含对象的slab描述符的地址
		 * slab页描述符的lru.prev字段指向相应的slab描述符
		 * */
		slabp = GET_PAGE_SLAB(virt_to_page(objp));
		/*从它的高速缓存链表(cachep->lists.slabs_partial或是cachep->lists.slabs_full上删除slab描述符)*/
		list_del(&slabp->list);
		/*计算slab内对象的下标*/
		objnr = (objp - slabp->s_mem) / cachep->objsize;
		check_slabp(cachep, slabp);
...
#endif
		/*
		 * 将slabp->free的当前值存放在对象描述符中,并将对象的下标放入slabp->free
		 * 最后被释放的对象将再次成为首次被分配的对象
		 * */
		...
		/*slab中所有对象空闲*/
		if (slabp->inuse == 0) {
			/*整个高速缓存中空闲对象的个数(lists.free_objects)>限制的个数(free_limit)*/
			if (cachep->lists.free_objects > cachep->free_limit) {
				cachep->lists.free_objects -= cachep->num;
				/*将slab页框释放到分区页框分配器*/
				slab_destroy(cachep, slabp);
			} else {
				/*将slab描述符插入到slabs_free链表中*/
				list_add(&slabp->list,
			 ...
			 */
			 ...
			 /*inuse > 0,slab被部分填充,将slab描述符插入到slabs_partial中*/
...
static void cache_flusharray (kmem_cache_t* cachep, struct array_cache *ac)
{
	...
	if (cachep->lists.shared) {
		...
		if (max) {
			if (batchcount > max)
				batchcount = max;
			/*从CPU的本地高速缓存上移ac->batchcount个指针来填充共享本地高速缓存*/
 ...
 * 参数:kmem_cache_t *cachep---高速缓存描述符的地址
 * 	void *objp---将被释放对象的地址
 * */
...
static void cache_reap(void *unused)
{
	...
	if (down_trylock(&cache_chain_sem)) {
		...
		/*信号量已获得，调用schedule_delayed_work()去调度该函数的下一次执行*/
		schedule_delayed_work(&__get_cpu_var(reap_work), REAPTIMEOUT_CPUC + smp_processor_id());
		return;
	}

	/*扫描存放在cache_chain链表中的kmem_cache_t描述符，*/
	list_for_each(walk, &cache_chain) {
		...
		/*SLAB_NO_REAP置位，页框回收被禁止，处理链表中的下一个高速缓存*/
		...
		/*清空局部slab高速缓存,则会有新的slab被释放  mm/slab.c*/
		drain_array_locked(searchp, ac_data(searchp), 0);

		/*
		 * 每个高速缓存都有"收割时间(reap time)", 该值存放在next_reap中，
		 * 若jiffies < next_reap，继续处理链表中的下一个高速缓存
		 * */
		...
		/*多处理器系统。清空slab共享高速缓存,则会有新的slab被释放  mm/slab.c*/
		if (searchp->lists.shared)
			drain_array_locked(searchp, searchp->lists.shared, 0);

		/*
		 * free_touched == 1---有新的slab最近被加入高速缓存
		 * 跳过这个高速缓存，继续处理链表中下一个高速缓存
		 * */
		...
		/*
		 * 经验公式:计算要释放的slab数量
		 * 基本上取决于高速缓存中空闲对象数的上限和能装入单个slab的对象数
		 * */
		...
		do {
			...
			/*对高速缓存空闲slab链表中的每个slab重复调用slab_destroy()，直到链表为空或者已回收目标数量的空闲slab*/
